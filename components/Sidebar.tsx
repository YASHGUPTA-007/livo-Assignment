    ...
    <>
      ...
      <div
        className={cn(
          ...
          "bg-sage-950 border-r border-sage-700/50 text-white"
        )}
      >
        {/* ── Header ── */}
        <div className="flex h-[72px] shrink-0 items-center justify-between px-6 border-b border-sage-700/50 md:justify-start">
          <div className="flex items-center gap-4 overflow-hidden w-full">
            ...
            {!isCollapsed && (
              <div className="flex flex-col transition-opacity duration-300">
                <span className="text-[15px] font-extrabold text-white tracking-wider leading-none mb-1">
                  LIVO AI
                </span>
                <span className="text-[10px] text-white/50 uppercase tracking-widest leading-none font-semibold">
                  Admin Panel
                </span>
              </div>
            )}
          ...
          <button
            type="button"
            className="text-white/60 hover:text-white transition-colors md:hidden shrink-0"
            onClick={() => setIsOpen(false)}
          >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden py-8 flex flex-col gap-8 custom-scrollbar">
          ...
          <div className="px-4">
            {!isCollapsed && (
              <h4 className="px-4 mb-3 text-[10px] font-bold uppercase tracking-widest text-white/40 transition-opacity duration-300">
                Overview
              </h4>
            )}
            <Link
              ...
              className={cn(
                ...
                pathname === "/dashboard"
                  ...
                  : "text-white/70 hover:text-white hover:bg-sage-700/30",
                isCollapsed ? "justify-center" : "gap-4"
              )}
              title={isCollapsed ? "Dashboard" : undefined}
            >
              <LayoutDashboard className={cn("shrink-0 transition-all duration-300", isCollapsed ? "h-6 w-6" : "h-5 w-5", pathname === "/dashboard" ? "text-black" : "text-white/60 group-hover:text-white")} />
          ...
          <div className="px-4">
            ...
            <div className="space-y-2">
              ...
              >
                <Mic className={cn("shrink-0 transition-all duration-300", isCollapsed ? "h-6 w-6" : "h-5 w-5", pathname === "/dashboard/practice" ? "text-black" : "text-white/60 group-hover:text-white")} />
              ...
              >
                <History className={cn("shrink-0 transition-all duration-300", isCollapsed ? "h-6 w-6" : "h-5 w-5", pathname === "/dashboard/history" ? "text-black" : "text-white/60 group-hover:text-white")} />
          ...
          <div className="px-4">
            ...
            >
              <Settings className={cn("shrink-0 transition-all duration-300", isCollapsed ? "h-6 w-6" : "h-5 w-5", pathname === "/dashboard/settings" ? "text-black" : "text-white/60 group-hover:text-white")} />
        ...
        <div className="px-4 pb-6 mt-auto">
          ...
          <button 
            ...
            className={cn(
              "hidden md:flex items-center px-4 py-3 text-[13px] font-bold text-white/60 hover:text-white hover:bg-sage-700/30 rounded-xl transition-all duration-300 mb-6 w-full group",
          ...
          <div className={cn(
            ...
            <div className={cn("flex items-center", isCollapsed ? "justify-center mb-0" : "gap-4 mb-4")}>
              ...
              {!isCollapsed && (
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  <span className="text-[11px] text-white/50 font-semibold flex items-center gap-1.5 mt-0.5 opacity-80">
            ...
            {!isCollapsed && (
              <div className="flex gap-2 border-t border-sage-700/30 pt-3 mt-1">
                <Link href="/dashboard/settings" className="flex-1 flex justify-center py-2 rounded-lg hover:bg-sage-700/30 text-white/60 hover:text-white transition-colors group">
                  <Settings className="h-4 w-4 group-hover:rotate-45 transition-transform duration-300" />
                </Link>
                <button onClick={signOut} className="flex-1 flex justify-center py-2 rounded-lg hover:bg-sage-700/30 text-white/60 hover:text-red-400 transition-colors group">
